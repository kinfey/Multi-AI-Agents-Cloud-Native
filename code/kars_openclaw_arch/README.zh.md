...
```text
  ...
  开发者 / CI
	  |
	  | azd up + Bicep                         构建和拉取镜像
	  v                                               |
  +-------------------+                    +------------v-------------+
  | Azure 资源组      |------------------->| Azure Container Registry |
  |                   |                    | KARS 与应用镜像           |
  +-------------------+                    +------------+-------------+
	  |                                                    |
	  | 创建资源                                           |
	  +---------------------------+------------------------+
			  ...
			  v                                   v
  +----------------------------------+  +----------------------------------+
  | Azure Container Apps 环境       |  | Azure Kubernetes Service (AKS)  |
  |                                  |  |                                  |
  |  +----------------------------+  |  |  +----------------------------+  |
  |  | Media App                  |  |  |  | KARS Controller            |  |
  |  | FastAPI + 视频 SPA         |  |  |  | CRD、策略与生命周期管理    |  |
  |  +-------------+--------------+  |  |  +-------------+--------------+  |
  |                ^                 |  |                |                 |
 |  +-------------+--------------+  |  |      管理所有三个              |
 |  | Red-team App               |  |  |      OpenClaw Sandboxes 作为  |
 |  | FastAPI + 报告 SPA         |  |  |      同级                   |
 |  +-------------+--------------+  |  |  +-------------v--------------+  |
  +----------------+-----------------+  |  | Media-Claw-Agent            |  |
			    ^                    |  | 08:00 调度器 + heartbeat     |  |
		...
		+-----------------+           |         |         |              |
					   ...
					   严格出口白名单策略|          |          | 遥测
									 v          v          v
  +-------------------+  +--------------------+  +-------------------------+
  | 新闻 / RSS 源     |  | Microsoft Foundry |  | Log Analytics +         |
  | 中美市场动态      |  | GPT + MAI Image   |  | Application Insights    |
  +-------------------+  +---------+----------+  +-------------------------+
						    |
					+--------v---------+
					| Azure Speech     |
					...
					+------------------+
						    ...
						    共享数据与身份平面
  +------------------------+  +----------------------+  +------------------+
  | 私有 Blob Storage      |  | Azure Key Vault      |  | Managed Identity |
  | media/                 |  | 图片与语音密钥       |  | OIDC 联合身份    |
  | redteam/               |  | RBAC，Git 无密钥     |  | 最小权限访问     |
  +-----------+------------+  +----------+-----------+  +---------+--------+
		    ^                          ^                        |
		    | manifest、媒体、        | 运行时读取密钥         |
		    | 点赞评分与测试报告       |                        |
		    +--------------------------+------------------------+
					  AKS Sandboxes 与 Container Apps
```

浏览器只通过 HTTPS 访问两个 Container Apps。Media-Claw-Agent 在受限的 KARS Sandbox 中执行每日生成流水线；Media-App-Agent 提供视频目录与互动数据；Media-Testing-Agent 处理五类安全评估。AKS Workload Identity 与 Container Apps Managed Identity 以最小权限访问私有 Azure 资源，仓库中不写入 tenant ID、subscription ID 或凭据。
